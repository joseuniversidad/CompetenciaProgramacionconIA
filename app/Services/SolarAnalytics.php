...
use App\Models\{Department,Farm,Generation};
use Carbon\Carbon;
class SolarAnalytics {
 public function summary(?int $departmentId=null,?string $period=null): array {
  ...
  $departments=Department::when($departmentId,fn($q)=>$q->whereKey($departmentId))->get()->map(function($d)use($farms,$generations){$f=$farms->where('department_id',$d->id);$g=$generations->whereIn('farm_id',$f->pluck('id'));return ['id'=>$d->id,'name'=>$d->name,'farms'=>$f->count(),'active_farms'=>$f->where('active',true)->count(),'panels'=>$f->sum('panel_count'),'capacity_kw'=>round($f->sum('capacity_kw'),3),'families'=>$f->sum('families'),'actual_kwh'=>round($g->sum('actual_kwh'),2),'expected_kwh'=>round($g->sum('expected_kwh'),2),'co2_kg'=>round($g->sum('actual_kwh')*0.4,2)];})->sortByDesc('actual_kwh')->values();
 ...
 public function forecast(Farm $farm): array {
  $history=$farm->generations()->orderBy('period')->get();$backtest=[];
  foreach($history as $i=>$g){if($i<3)continue;$previous=$history->slice($i-3,3)->values();if(Carbon::parse($previous[0]->period)->addMonths(3)->toDateString()!==$g->period)continue;$prediction=round($previous->avg('actual_kwh'),2);$backtest[]=['period'=>substr($g->period,0,7),'projected_kwh'=>$prediction,'actual_kwh'=>$g->actual_kwh,'absolute_error'=>round(abs($prediction-$g->actual_kwh),2)];}
  $tail=$history->take(-3)->values();$complete=$tail->count()===3&&Carbon::parse($tail[0]->period)->addMonths(2)->toDateString()===$tail[2]->period;
